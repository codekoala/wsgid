...
class IAppLoader(plugnplay.Interface):
    ...
    def load_app(self, app_path, app_full_name):
        ...
        @ap_path: Path on disk where the app is located. Already inserted into
                  sys.path. This is --app-path + 'app/'
...
def load_app(app_path, wsgi_app_full_name):
    ...
    raise Exception("No Loader found for app %s and no --wsgi-app option "
                    "found\n" % app_path)
...
class PyRoutesLoader(Plugin):
    ...
    def load_app(self, app_path, app_full_name):
        ...
        dirs = [d for d in os.listdir(app_path)
                if os.path.isdir(os.path.join(app_path, d))]

        # A normal pyroutes application should have only 3 folders: templates/
        # tests/ <app-name>/

        # Here we do: "import <app-name>"
        # TODO: Switch to import_module
